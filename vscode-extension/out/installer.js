...
function httpsGet(url) {
  return new Promise((resolve, reject) => {
    const req = https.get(
      url,
      { headers: { "User-Agent": "ptrm-vscode" } },
      (res) => {
...
function downloadFile(url, dest) {
  return new Promise((resolve, reject) => {
    const follow = (u) => {
      https
        .get(u, { headers: { "User-Agent": "ptrm-vscode" } }, (res) => {
          if (res.statusCode === 301 || res.statusCode === 302) {
            const loc = res.headers.location;
            if (loc) {
              return follow(loc);
  ...
  await vscode.window.withProgress(
    {
      ...
      title: `Installing ptrm v${version}...`,
      cancellable: false,
    },
    async (progress) => {
...
async function ensureInstalled() {
  ...
  if (!installed || !currentVersion) {
    const action = await vscode.window.showWarningMessage(
      "ptrm CLI is not installed. Install it now?",
      "Install",
      "Later",
    );
    if (action !== "Install") {
    ...
    try {
      ...
      vscode.window.showInformationMessage(
        `ptrm v${latest} installed to ${binPath}`,
      );
      return true;
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      vscode.window.showErrorMessage(`Failed to install ptrm: ${msg}`);
      return false;
    }
  }
  return true;
...
async function checkForUpdate() {
  try {
    ...
    const action = await vscode.window.showInformationMessage(
      `ptrm v${latest} is available (current: v${current}). Update now?`,
      "Update",
      "Later",
    );
    if (action !== "Update") {
    ...
    const binPath = await installOrUpdate(latest);
    vscode.window.showInformationMessage(
      `ptrm updated to v${latest} at ${binPath}`,
    );
  } catch {
