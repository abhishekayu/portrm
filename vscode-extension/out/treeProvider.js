...
exports.PtrmTreeProvider = void 0;
const vscode = __importStar(require("vscode"));
const utils_1 = require("./utils");
// ── Provider ───────────────────────────────────────────────────────
class PtrmTreeProvider {
    constructor() {
    ...
    async loadData() {
        ...
        try {
            this._projectMode = (0, utils_1.hasConfig)();
            this._projectName = (0, utils_1.getProjectName)();
            if (this._projectMode) {
                this._services = await (0, utils_1.runPtrmJson)("status --json");
                try {
                    this._ports = await (0, utils_1.runPtrmJson)("scan --json");
                }
                catch (e) {
    ...
    buildServiceItem(svc) {
        ...
        let description = `${(0, utils_1.formatPort)(svc.port)} - ${statusLabel}`;
        if (svc.process) {
    ...
    buildPortItem(info) {
        const service = (0, utils_1.formatServiceLabel)(info);
        const pid = info.process?.pid;
        const mem = (0, utils_1.formatMemory)(info.process?.memory_bytes);
        const uptime = (0, utils_1.formatUptime)(info.process?.runtime);
    ...
    buildPortTooltip(info) {
        ...
        if (info.process) {
            ...
            if (p.memory_bytes) {
                lines.push(`| **Memory** | ${(0, utils_1.formatMemory)(p.memory_bytes)} |`);
            }
            if (p.cpu_usage !== undefined) {
                lines.push(`| **CPU** | ${p.cpu_usage.toFixed(1)}% |`);
            }
            if (p.runtime) {
                lines.push(`| **Uptime** | ${(0, utils_1.formatUptime)(p.runtime)} |`);
            }
            if (p.working_dir) {
                lines.push(`| **CWD** | ${p.working_dir} |`);
            }
        }
...
exports.PtrmTreeProvider = PtrmTreeProvider;
